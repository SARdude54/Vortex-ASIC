...
`define VX_WRITEBACK_IF_SIGNALS(prefix) \
    logic       prefix``_valid; \
    writeback_t prefix``_data

`define VX_WRITEBACK_IF_SIGNALS_N(prefix, N) \
    logic [(N)-1:0] prefix``_valid; \
    writeback_t    prefix``_data [(N)-1:0]

`define VX_WRITEBACK_IF_PRODUCER_PORTS(prefix) \
    output logic       prefix``_valid, \
    output writeback_t prefix``_data

`define VX_WRITEBACK_IF_PRODUCER_PORTS_N(prefix, N) \
    output logic [(N)-1:0] prefix``_valid, \
    output writeback_t     prefix``_data [(N)-1:0]

`define VX_WRITEBACK_IF_CONSUMER_PORTS(prefix) \
    input logic       prefix``_valid, \
    input writeback_t prefix``_data

`define VX_WRITEBACK_IF_CONSUMER_PORTS_N(prefix, N) \
...
`define VX_WRITEBACK_IF_PASS_PORTS_N(def_pref, pass_pref) \
    .``def_pref``_valid(pass_pref``_valid), \
    .``def_pref``_data(pass_pref``_data)

`define VX_WRITEBACK_IF_PASS_PORTS_I(def_pref, pass_pref, i) \
