...
`define VX_LSU_MEM_IF_SIGNALS_N(prefix, NUM_LANES, DATA_SIZE, TAG_WIDTH, FLAGS_WIDTH, MEM_ADDR_WIDTH, N) \
    localparam prefix``_ADDR_WIDTH = (MEM_ADDR_WIDTH) - `CLOG2(DATA_SIZE); \
    logic [(N)-1:0] prefix``_req_valid; \
    logic [(N)-1:0][(NUM_LANES)-1:0] prefix``_req_data_mask; \
    logic [(N)-1:0] prefix``_req_data_rw; \
    logic [(N)-1:0][(NUM_LANES)-1:0][prefix``_ADDR_WIDTH-1:0] prefix``_req_data_addr; \
    logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)*8-1:0] prefix``_req_data_data; \
    logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)-1:0] prefix``_req_data_byteen; \
    logic [(N)-1:0][(NUM_LANES)-1:0][(FLAGS_WIDTH)-1:0] prefix``_req_data_flags; \
    ...
    logic [(N)-1:0][(NUM_LANES)-1:0] prefix``_rsp_data_mask; \
    logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)*8-1:0] prefix``_rsp_data_data; \
...
`define VX_LSU_MEM_IF_SIGNALS_AW_N(prefix, NUM_LANES, DATA_SIZE, TAG_WIDTH, FLAGS_WIDTH, ADDR_WIDTH, N) \
    logic [(N)-1:0] prefix``_req_valid; \
    logic [(N)-1:0][(NUM_LANES)-1:0] prefix``_req_data_mask; \
    logic [(N)-1:0] prefix``_req_data_rw; \
    logic [(N)-1:0][(NUM_LANES)-1:0][(ADDR_WIDTH)-1:0] prefix``_req_data_addr; \
    logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)*8-1:0] prefix``_req_data_data; \
    logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)-1:0] prefix``_req_data_byteen; \
    logic [(N)-1:0][(NUM_LANES)-1:0][(FLAGS_WIDTH)-1:0] prefix``_req_data_flags; \
    logic [(N)-1:0][`UP(UUID_WIDTH)-1:0] prefix``_req_data_tag_uuid; \
...
`define VX_LSU_MEM_IF_PRODUCER_PORTS_N(prefix, NUM_LANES, DATA_SIZE, TAG_WIDTH, FLAGS_WIDTH, MEM_ADDR_WIDTH, N) \
    output logic [(N)-1:0] prefix``_req_valid, \
    output logic [(N)-1:0][(NUM_LANES)-1:0] prefix``_req_data_mask, \
    output logic [(N)-1:0] prefix``_req_data_rw, \
    output logic [(N)-1:0][(NUM_LANES)-1:0][(MEM_ADDR_WIDTH)-`CLOG2(DATA_SIZE)-1:0] prefix``_req_data_addr, \
    output logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)*8-1:0] prefix``_req_data_data, \
    output logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)-1:0] prefix``_req_data_byteen, \
    output logic [(N)-1:0][(NUM_LANES)-1:0][(FLAGS_WIDTH)-1:0] prefix``_req_data_flags, \
    ...
    input  logic [(N)-1:0][(NUM_LANES)-1:0] prefix``_rsp_data_mask, \
    input  logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)*8-1:0] prefix``_rsp_data_data, \
...
`define VX_LSU_MEM_IF_PRODUCER_PORTS_AW_N(prefix, NUM_LANES, DATA_SIZE, TAG_WIDTH, FLAGS_WIDTH, ADDR_WIDTH, N) \
    output logic [(N)-1:0] prefix``_req_valid, \
    output logic [(N)-1:0][(NUM_LANES)-1:0] prefix``_req_data_mask, \
    output logic [(N)-1:0] prefix``_req_data_rw, \
    output logic [(N)-1:0][(NUM_LANES)-1:0][(ADDR_WIDTH)-1:0] prefix``_req_data_addr, \
    output logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)*8-1:0] prefix``_req_data_data, \
    output logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)-1:0] prefix``_req_data_byteen, \
    output logic [(N)-1:0][(NUM_LANES)-1:0][(FLAGS_WIDTH)-1:0] prefix``_req_data_flags, \
    output logic [(N)-1:0][`UP(UUID_WIDTH)-1:0] prefix``_req_data_tag_uuid, \
...
`define VX_LSU_MEM_IF_CONSUMER_PORTS_N(prefix, NUM_LANES, DATA_SIZE, TAG_WIDTH, FLAGS_WIDTH, MEM_ADDR_WIDTH, N) \
    input  logic [(N)-1:0] prefix``_req_valid, \
    input  logic [(N)-1:0][(NUM_LANES)-1:0] prefix``_req_data_mask, \
    input  logic [(N)-1:0] prefix``_req_data_rw, \
    input  logic [(N)-1:0][(NUM_LANES)-1:0][(MEM_ADDR_WIDTH)-`CLOG2(DATA_SIZE)-1:0] prefix``_req_data_addr, \
    input  logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)*8-1:0] prefix``_req_data_data, \
    input  logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)-1:0] prefix``_req_data_byteen, \
    input  logic [(N)-1:0][(NUM_LANES)-1:0][(FLAGS_WIDTH)-1:0] prefix``_req_data_flags, \
    ...
    output logic [(N)-1:0][(NUM_LANES)-1:0] prefix``_rsp_data_mask, \
    output logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)*8-1:0] prefix``_rsp_data_data, \
...
`define VX_LSU_MEM_IF_CONSUMER_PORTS_AW_N(prefix, NUM_LANES, DATA_SIZE, TAG_WIDTH, FLAGS_WIDTH, ADDR_WIDTH, N) \
    input  logic [(N)-1:0] prefix``_req_valid, \
    input  logic [(N)-1:0][(NUM_LANES)-1:0] prefix``_req_data_mask, \
    input  logic [(N)-1:0] prefix``_req_data_rw, \
    input  logic [(N)-1:0][(NUM_LANES)-1:0][(ADDR_WIDTH)-1:0] prefix``_req_data_addr, \
    input  logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)*8-1:0] prefix``_req_data_data, \
    input  logic [(N)-1:0][(NUM_LANES)-1:0][(DATA_SIZE)-1:0] prefix``_req_data_byteen, \
    input  logic [(N)-1:0][(NUM_LANES)-1:0][(FLAGS_WIDTH)-1:0] prefix``_req_data_flags, \
    input  logic [(N)-1:0][`UP(UUID_WIDTH)-1:0] prefix``_req_data_tag_uuid, \
...
`define VX_LSU_MEM_IF_PASS_PORTS_I(def_pref, pass_pref, i) \
    .``def_pref``_req_valid(pass_pref``_req_valid[i]), \
    .``def_pref``_req_data_mask(pass_pref``_req_data_mask[i]), \
    ...
    .``def_pref``_rsp_data_mask(pass_pref``_rsp_data_mask[i]), \
...
`define VX_LSU_MEM_IF_PASS_PORTS_SLICE(def_pref, pass_pref, base, N) \
    .``def_pref``_req_valid(pass_pref``_req_valid[(base) +: (N)]), \
    .``def_pref``_req_data_mask(pass_pref``_req_data_mask[(base) +: (N)]), \
    ...
    .``def_pref``_rsp_data_mask(pass_pref``_rsp_data_mask[(base) +: (N)]), \
...
// Slice helpers
`define VX_LSU_MEM_IF_SLICE_REQ_VALID(prefix, i) \
    prefix``_req_valid[i]

`define VX_LSU_MEM_IF_SLICE_REQ_DATA_MASK(prefix, i) \
    prefix``_req_data_mask[i]

`define VX_LSU_MEM_IF_SLICE_REQ_DATA_RW(prefix, i) \
    prefix``_req_data_rw[i]

`define VX_LSU_MEM_IF_SLICE_REQ_DATA_ADDR(prefix, i) \
    prefix``_req_data_addr[i]

`define VX_LSU_MEM_IF_SLICE_REQ_DATA_DATA(prefix, i) \
    prefix``_req_data_data[i]

`define VX_LSU_MEM_IF_SLICE_REQ_DATA_BYTEEN(prefix, i) \
    prefix``_req_data_byteen[i]

`define VX_LSU_MEM_IF_SLICE_REQ_DATA_FLAGS(prefix, i) \
    prefix``_req_data_flags[i]

`define VX_LSU_MEM_IF_SLICE_REQ_DATA_TAG_UUID(prefix, i) \
    prefix``_req_data_tag_uuid[i]

`define VX_LSU_MEM_IF_SLICE_REQ_DATA_TAG_VALUE(prefix, i) \
    prefix``_req_data_tag_value[i]

`define VX_LSU_MEM_IF_SLICE_REQ_READY(prefix, i) \
    prefix``_req_ready[i]

`define VX_LSU_MEM_IF_SLICE_RSP_VALID(prefix, i) \
    prefix``_rsp_valid[i]

`define VX_LSU_MEM_IF_SLICE_RSP_DATA_MASK(prefix, i) \
    prefix``_rsp_data_mask[i]

`define VX_LSU_MEM_IF_SLICE_RSP_DATA_DATA(prefix, i) \
    prefix``_rsp_data_data[i]

`define VX_LSU_MEM_IF_SLICE_RSP_DATA_TAG_UUID(prefix, i) \
    prefix``_rsp_data_tag_uuid[i]

`define VX_LSU_MEM_IF_SLICE_RSP_DATA_TAG_VALUE(prefix, i) \
    prefix``_rsp_data_tag_value[i]

`define VX_LSU_MEM_IF_SLICE_RSP_READY(prefix, i) \
    prefix``_rsp_ready[i]
